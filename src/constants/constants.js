export const projects = [
  {
    title: 'E-Commerce',
    description: "An E-Commerce using React.js and handling with Commercejs API and Stripe API",
    image: '/images/ecommerce.PNG',
    tags: ['React, Commercejs, Stripe, Material-UI'],
    source: 'https://github.com/Deadflight/e-commercejs-react',
    visit: 'https://ecommercejsreact.netlify.app/',
    id: 0
  },
  {
    title: 'Rest Countries API',
    description: "Using React, I made an application using REST Countries API to show the Countries with their respective descriptions to the users ",
      image: '/images/restcountriesapi.PNG',
      tags: ['React','API REST'],
    source: 'https://github.com/Deadflight/rest-countries-api',
    visit: 'https://rest-api-countries-deadflight.netlify.app',
    id: 1,
  },
  {
    title: 'Travel Companion',
    description: "Travel Companion, an React app, working with Travel Advisor API, Google Maps and Open Weather Map API",
    image : '/images/travelcompanion.PNG',
    tags: ['React, API REST, Google Maps, Material-UI'],
    source: 'https://github.com/Deadflight/travel-companion-react',
    visit: 'https://travelcompaniondeadflight.netlify.app',
    id: 2
  },
  {
    title: 'Sunnyside Agency',
    description:"With Vanilla JavaScript, I completed this challenge from frontendmentor.io",
    image: '/images/sunnyside.PNG',
    tags: ['HTML5', 'SASS', 'Vanilla JavaScript'],
    source: 'https://github.com/Deadflight/sunnyside-agency',
    visit: 'https://deadflight.github.io/sunnyside-agency/',
    id: 3,
  },
  {
    title: 'Blogr',
    description: "Blogr using HTML5, CSS3, Sass, JavaScript. Challenge frontendmentor",
      image: '/images/blogr.PNG',
      tags: ['HTML5', 'SASS', 'Vanilla JavaScript'],
    source: 'https://github.com/Deadflight/blogr',
    visit: 'https://deadflight.github.io/blogr/',
    id: 4,
  },
  {
    title: 'Easybank',
    description: "This is a solution to the easybank landing page challenge on Frontend Mentor using HTML5, CSS3, Sass, Vanilla JavaScript.",
    image: '/images/easybank.PNG',
    tags: ['HTML5', 'SASS', 'Vanilla JavaScript'],
    source: 'https://github.com/Deadflight/easybank-landing-page-master',
    visit: 'https://deadflight.github.io/easybank-landing-page-master/',
    id: 5,
  },
];

export const TimeLineData = [
  { year: 2017, text: 'Started my journey', },
  { year: 2018, text: 'Worked as a freelance developer', },
  { year: 2019, text: 'Founded JavaScript Mastery', },
  { year: 2020, text: 'Shared my projects with the world', },
  { year: 2021, text: 'Started my own platform', },
];

export const MetaTags = {
  og: {
    title: 'Carlos Correa',
    image: 'https://github.com/Deadflight/portfolio-react/blob/main/public/images/ScreenShot.PNG',
    description: 'Carlos Correa Portfolio',
    url: 'https://carloscorreaportfolio.netlify.app/'
  }
};